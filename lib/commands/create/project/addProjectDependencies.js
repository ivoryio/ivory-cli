...
module.exports = () =>
  addDevDependencies().pipe(
    concatMap(addLocalDependencies),
...
const addDevDependencies = () =>
  Observable.create(observer => {
    ...
    shell.exec(cmd, { silent: true }, code => {
      if (code !== 0) {
        observer.error(new Error('Failed to add dev dependencies'))
      } else {
        observer.next()
        observer.complete()
      }
...
const addLocalDependencies = () =>
  Observable.create(observer => {
    ...
    shell.exec(cmd, { silent: true }, code => {
      if (code !== 0) {
        observer.error(new Error('Failed to add global dependencies'))
      } else {
        observer.next()
        observer.complete()
      }
