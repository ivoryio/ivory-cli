...
module.exports = {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        cf.listStacks({}, function (err) {
          ...
          log.succeed('Credentials valid! (please make sure you have the right permission for AWS)')
          resolve()
        })
      } catch (err) {
        log.fail()
  ...
  deployStack: function (fsm) {
    ...
    return new Promise((resolve, reject) => {
      try {
        shell.cd(`${fsm.projectName}/src/pods/user/api/`)
        createBuildDirectory()

        const s3 = new AWS.S3()
        const bucketName = `${fsm.projectName}-builds`
