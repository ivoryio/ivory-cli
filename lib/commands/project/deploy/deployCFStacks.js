...
module.exports = function (projectProperties) {
  return new Promise((resolve, reject) => {
    ...
    function onAnswer (answer) {
      ...
      if ((fsm.is('defaultAWSProfile') || fsm.is('namedAWSProfile')) && answer.name === 'awsRegion') {
        ...
        aws.configure(fsm)
          ...
          .then(stackOutputs => amplify.configure(fsm, stackOutputs).then(() => stackOutputs))
          .then(complete)
          .catch(err => {
      ...
      function complete (stackOutputs) {
        fsm.completeWithSuccess(stackOutputs)
        prompt.done()
      }
    }
    ...
    function onComplete () {
      if (fsm.is('success')) {
        return resolve({...projectProperties, userPoolARN: fsm.stackOutputs.find((out) => out.OutputKey === 'UserPoolArn').OutputValue})
      }
