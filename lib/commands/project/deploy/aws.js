    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        s3.createBucket({ Bucket: bucketName }, (err) => {
          ...
          log.text = `Running command: ${packageCmd}`
          shell.exec(packageCmd, { silent: true }, (code, stdout, stderr) => {
            ...
            const deployCmd = `aws cloudformation deploy --template-file build/infrastructure.packaged.yaml --capabilities CAPABILITY_IAM --stack-name ${stackName} --parameter-overrides StageName=dev AuthPrefix=${fsm.projectName}user --profile ${fsm.awsCredentials.profile} `
            log.text = `Running command: ${deployCmd}`
