...
module.exports = (log) => (podName) => readProjectProperties().pipe(
  tap(logDeployPod(log, podName)),
  concatMap(packageCFTemplate(podName)),
  concatMap(deployCFTemplate(podName)),
  concatMap(updateAWSConfig),
  concatMap(decideWhatToConfigure(podName)),
...
const decideWhatToConfigure = (podName) => (projectProperties) => iif(
  () => podName === 'user',
...
const configureAppApi = (podName, { name }) => Observable.create(observer => {
  const projectPrefix = `ivory-${name}`
  const stackName = `${projectPrefix}-dev-${podName}`
...
const logRunCustomScripts = (log) => (podName) => {
