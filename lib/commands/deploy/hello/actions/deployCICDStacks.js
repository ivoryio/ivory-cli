  ...
  Observable.create(observer => {
      ...
      route53.listHostedZones({}, (err, data) => {
        ...
        acm.listCertificates(params, (err, data) => {
          ...
          if (!certificate) {
            return observer.error(
              new Error(
                `No issued certificate found in AWS Certificate Manager for ${config.domainName}`
              )
            )
          }
    ...
    function runCDKDeploy (hostedZoneId, domainName, certificateARN) {
      ...
      if (hostedZoneId && domainName && certificateARN) {
        cmd = `npm i && PROJECT_NAME=${projectName} \
REGION=${config.region} \
HOSTED_ZONE_ID=${hostedZoneId} \
APP_DOMAIN_NAME=${domainName} \
CERTIFICATE_ARN=${certificateARN} \
npx cdk@0.34.0 deploy --profile ${config.profile} --require-approval never`
      } else {
        cmd = `npm i && PROJECT_NAME=${projectName} \
REGION=${config.region} \
npx cdk@0.34.0 deploy --profile ${config.profile} --require-approval never`
      ...
      shell.exec(cmd, { silent: true }, (code, _, stderr) => {
        ...
        shell.cd('..')
        observer.next({ ...config, projectName, deployCmd: cmd })
        observer.complete()
      })
    }
  })
